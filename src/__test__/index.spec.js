  ...
  describe('for packages without workspaces', () => {
    describe('without configuration', () => {
      ...
      it('runs the scripts for test/lint/typecheck when they are defined', async () => {
        ...
        expect(execa).toHaveBeenCalledWith('npm', ['run', 'typecheck'], {
          cwd: root,
        })
      })
    ...
    describe('with configuration', () => {
      ...
      it('matches patterns in file names to custom task criteria', async () => {
        ...
        expect(execa).toHaveBeenCalledWith('do', ['this', 'this.xy'], {
          cwd: root,
        })
        expect(execa).toHaveBeenCalledWith(
          'do',
          ['that', 'that.xx', 'that.yy'],
          {cwd: root},
        )
        expect(execa).toHaveBeenCalledWith(
          'do',
          ['all', 'this.xy', 'that.xx', 'that.yy', 'other.js'],
          {cwd: root},
        )
      })
    })
  ...
  describe('for packages with workspaces', () => {
    describe('without configuration', () => {
      it('does not run any commands when the workspace does not have staged files', async () => {
        ...
        await fs.writeFile(
          path.join(root, 'w1', 'package.json'),
          JSON.stringify(w1Json),
        )
        await fs.writeFile(
          path.join(root, 'w2', 'package.json'),
          JSON.stringify(w2Json),
        )
        await monotaskr({cwd: root})
      ...
      it('runs the scripts for test/lint/typecheck when they are defined', async () => {
        ...
        expect(execa).toHaveBeenCalledWith(
          'npm',
          ['run', 'lint', '--', 'index.ts'],
          {cwd: path.join(root, 'w1')},
        )
        expect(execa).toHaveBeenCalledWith('npm', ['run', 'test'], {
          cwd: path.join(root, 'w1'),
        })
        expect(execa).toHaveBeenCalledWith('npm', ['run', 'typecheck'], {
          cwd: path.join(root, 'w1'),
    ...
    describe('with configuration', () => {
      it('runs the specified tasks', async () => {
        ...
        expect(execa).toHaveBeenCalledWith('do', ['this'], {
          cwd: path.join(root, 'w1'),
        })
        expect(execa).toHaveBeenCalledWith('do', ['that'], {
          cwd: path.join(root, 'w1'),
        })
      })
      ...
      it('uses custom stages in the specified order', async () => {
        ...
        expect(execa.mock.calls[1]).toEqual([
          'do',
          ['that', 'with', 'package.json', 'index.js'],
          {cwd: path.join(root, 'w1')},
        ])
        expect(execa.mock.calls[2]).toEqual(['rooting', [], {cwd: root}])
        expect(execa.mock.calls[3]).toEqual([
          'do',
          ['this'],
          {cwd: path.join(root, 'w1')},
        ])
      ...
      it('matches patterns in file names to custom task criteria', async () => {
        ...
        expect(execa).toHaveBeenCalledWith(
          'do',
          ['w1', 'json', 'w1/package.json'],
          {cwd: root},
        )
        expect(execa).toHaveBeenCalledWith(
          'do',
          ['all', 'w1', 'package.json', 'index.js', 'other.file'],
          {cwd: path.join(root, 'w1')},
        )
        expect(execa).toHaveBeenCalledWith('do', ['w1', 'js', 'index.js'], {
          cwd: path.join(root, 'w1'),
        })
        expect(execa).toHaveBeenCalledWith('do', ['w2', 'ts', 'index.ts'], {
          cwd: path.join(root, 'w2'),
        })
      })
    })
  })
