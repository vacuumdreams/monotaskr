...
const getDefaultMainConfig = (packageJson) => ({
  ...
  tasks:
    packageJson.workspaces?.length || Boolean(packageJson.monotaskr?.stages)
      ? []
      : getDefaultTasks(ROOT_STAGE, packageJson),
  ...packageJson.monotaskr,
})

const getScopedFiles = (root, files) =>
  files
...
const addTask = ({tasksConfig, task, packageName, root, files, stage}) => {
  if (!tasksConfig[task.stage ?? stage]) {
    throw new Error(
      `The stage ${chalk.bold(
        `"${task.stage}"`,
      )} does not exist. Make sure you provide a configuration for it in your root.`,
    )
  }
...
module.exports = async ({config, files: allFiles}) => {
  ...
  const allTasks = (config.workspaces ?? []).reduce((tasksConfig, wsConfig) => {
    const {tasks} = {
      tasks: getWsTasks(
        Boolean(config.main.pjson.monotaskr?.stages),
        wsConfig.pjson,
      ),
